...
| **2** | Snowsight (Web IDE) | Data discovery using phData Forge and dbt model development using CoCo | 45 min |
...
3. Prompt CoCo to generate sample data to populate the tables it just created.
4. Prompt CoCo to use the same GIT API Integration to upload the business PDF `northwind.pdf` into a Snowflake internal stage
5. Prompt CoCo to use the same GIT API Integration to import `top_queries.sql`
6. Open your terminal
7. Navigate to your project directory:
   ```bash
   cd COCO_HOL
   ```
8. Install the phData CLI (if you have not done so already):
   ```
   ...
   ```
9. Create a new toolkit instance within the directory:
   ```bash
   toolkit init
   ```
10. Now we will configure the phData Toolkit to Snowflake and the Northwinds database:
   - Open `toolkit.conf`
